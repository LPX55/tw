...
(function(root) {
	...
	var inpMiddleClassCodeId = 'inpMiddleClassCode' + Math.random().toString(36).substr(2, 6);
	var inpMiddleClassNameId = 'inpMiddleClassName' + Math.random().toString(36).substr(2, 6);
			...
			result = {
				...
				"initialize": function(options) {
					...
					var tag = '<div class="row" style="margin-top: 5px;">'
									...
									+ '      <label class="control-label col-sm-2" for="' + inpMiddleClassCodeId + '">代碼：</label>'
									+ '	     <div class="col-sm-4">'
									+ '        <input type="text" class="form-control" id="' + inpMiddleClassCodeId + '" data-field-name="middle_class_code" disabled>'
									+ '      </div>'
									+ '      <label class="control-label col-sm-2" for="' + inpMiddleClassNameId + '">名稱：</label>'
									+ '      <div class="col-sm-4">'
									+ '        <input type="text" class="form-control" id="' + inpMiddleClassNameId + '" data-field-name="middle_class_name" disabled>'
									+ '      </div>'
									+ '    </div>'
									+ '  </form>'
									+ '</div>';
				...
				"navButtonClickTrigger": function(btnName) {
					...
					function checkFieldValue() {
						...
						if ((booCheckFieldResult == true) && (jQuery('#' + inpMiddleClassCodeId).val() == '')) {

							booCheckFieldResult = false;
							
							FormUtils.showMessage('代碼不可空白！', function() {

								jQuery('#' + inpMiddleClassCodeId).focus();
							});
						}
						
						if ((booCheckFieldResult == true) && (jQuery('#' + inpMiddleClassNameId).val() == '')) {

							booCheckFieldResult = false;
							
							FormUtils.showMessage('名稱不可空白！', function() {

								jQuery('#' + inpMiddleClassNameId).focus();
							});
						}
					}
				...
				"render": function() {
					...
					if (this.getFormState() === 'update') {
						...
						jQuery('#' + inpMiddleClassCodeId).prop('disabled', true);
					}
