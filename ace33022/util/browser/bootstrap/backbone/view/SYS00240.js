...
(function(root) {
	...
	var inpSugarGroupNameId = 'inpSugarGroupName' + Math.random().toString(36).substr(2, 6);
			...
			result = {
				...
				"initialize": function(options) {
					...
					var tag = '<div class="row" style="margin-top: 5px;">'
									...
									+ '    </div>'
									+ '    <div class="form-group">'
									+ '      <label class="control-label col-sm-2" for="' + inpSugarGroupCodeId + '">代碼：</label>'
									+ '	     <div class="col-sm-4">'
									+ '        <input type="text" class="form-control" id="' + inpSugarGroupCodeId + '" data-field-name="sugar_group_code" disabled>'
									+ '      </div>'
									+ '      <label class="control-label col-sm-2" for="' + inpSugarGroupNameId + '">名稱：</label>'
									+ '      <div class="col-sm-4">'
									+ '        <input type="text" class="form-control" id="' + inpSugarGroupNameId + '" data-field-name="sugar_group_name" disabled>'
									+ '      </div>'
									+ '    </div>'
									+ '  </form>'
									+ '</div>';
				...
				"navButtonClickTrigger": function(btnName) {
					...
					function checkFieldValue() {
						...
						if ((booCheckFieldResult == true) && (jQuery('#' + inpSugarGroupCodeId).val() == '')) {
							...
							FormUtils.showMessage('代碼不可空白！', function() { jQuery('#' + inpSugarGroupCodeId).focus(); });
						}

						if ((booCheckFieldResult == true) && (jQuery('#' + inpSugarGroupNameId).val() == '')) {

							booCheckFieldResult = false;

							FormUtils.showMessage('名稱不可空白！', function() { jQuery('#' + inpSugarGroupNameId).focus(); });
						}
					}
				...
				"render": function() {
					...
					if (this.getFormState() === 'browse') {
						...
						jQuery('#' + inpSugarGroupCodeId).prop('disabled', true);
					}
