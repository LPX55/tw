...
(function(root) {
	...
	var inpIceDosageGroupNameId = 'inpIceDosageGroupName' + Math.random().toString(36).substr(2, 6);
			...
			result = {
				...
				"initialize": function(options) {
					...
					var tag = '<div class="row" style="margin-top: 5px;">'
									...
									+ '    </div>'
									+ '    <div class="form-group">'
									+ '      <label class="control-label col-sm-2" for="' + inpIceDosageGroupCodeId + '">代碼：</label>'
									+ '	     <div class="col-sm-4">'
									+ '        <input type="text" class="form-control" id="' + inpIceDosageGroupCodeId + '" data-field-name="ice_dosage_group_code" disabled>'
									+ '      </div>'
									+ '      <label class="control-label col-sm-2" for="' + inpIceDosageGroupNameId + '">名稱：</label>'
									+ '      <div class="col-sm-4">'
									+ '        <input type="text" class="form-control" id="' + inpIceDosageGroupNameId + '" data-field-name="ice_dosage_group_name" disabled>'
									+ '      </div>'
									+ '    </div>'
									+ '  </form>'
									+ '</div>';
				...
				"navButtonClickTrigger": function(btnName) {
					...
					function checkFieldValue() {
						...
						if ((booCheckFieldResult == true) && (jQuery('#' + inpIceDosageGroupCodeId).val() == '')) {
							...
							FormUtils.showMessage('代碼不可空白！', function() { jQuery('#' + inpIceDosageGroupCodeId).focus(); });
						}

						if ((booCheckFieldResult == true) && (jQuery('#' + inpIceDosageGroupNameId).val() == '')) {

							booCheckFieldResult = false;

							FormUtils.showMessage('名稱不可空白！', function() { jQuery('#' + inpIceDosageGroupNameId).focus(); });
						}
					}
				...
				"render": function() {
					...
					if (this.getFormState() === 'browse') {
						...
						jQuery('#' + inpIceDosageGroupCodeId).prop('disabled', true);
					}
